...
function GIFAPI(sx) {// searchTerm
  ...
  fetch(gif_url).then(resp => resp.json()//response
    ...
    .then(cont => {
      //handel th data,pagination, meta
      //setgifs(conts.data); 
      renderGifs();
      gifs = cont.data;
      isloading = false;
...
function renderGifs() { //Good
  // show the loading  or the gifs
  if (isloading) {
    document.getElementById("disply_loader").innerHTML = "";
    let load = document.createElement('load');
    ...
    let out = document.querySelector('.disply_load');//change it to meaningfull name
