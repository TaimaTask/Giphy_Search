...
let gifButton = document.getElementById("gifButton");// Get the button that opens the modal
...
function searchfunc() { page=1;
  ...
  if (searchText == "") {xIcon.style.visibility = "hidden";gifApi("");}
  else {
...
fetch(gifUrl).then(response=>response.json()
  ...
  .then(content=>{//handel the data,pagination, meta
    console.log(content);
    //renderGifs();
    gifs=content.data;
    setTimeout(function(){
...
function LoadingGifs(){
  document.getElementById("loader").innerHTML = "";
    ...
    document.querySelector('.loader').insertAdjacentElement('beforeend',showLoader);
    setTimeout(function(){
        renderGifs();
  }, 1100)
}
